...
export class Axios {
  ...
  constructor(defaultConfig: InternalAxiosRequestConfig, interceptorOption: SetupInterceptorArgs) {
    this.instance = axios.create(defaultConfig);
    this.setupInterceptors(interceptorOption);
  }
  ...
  setupInterceptors({ request, response }: SetupInterceptorArgs): void {
    if (is2DArrays(request)) {
    ...
    if (is2DArrays(response)) {
      for (const iterator of response) {
        this.instance.interceptors.response.use(...iterator);
      }
    } else {
